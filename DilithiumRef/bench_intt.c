...
extern void REF(invntt_tomont)(int32_t a[N]);
extern void REF(ntt)(int32_t a[N]);
extern void AVX(invntt_avx)(__m256i *a, const __m256i *qdata);
...
static int32_t diff_mod_q(int32_t a, int32_t b) {
  int64_t r = ((int64_t)a - b) % Q;
  if (r < 0) {
    r += Q;
  }
  return (int32_t)r;
...
static void prepare_intt_inputs(poly_t *ref_input, poly_t *avx_input) {
  poly_t plain;
  ...
  *avx_input = plain;
  ...
  REF(ntt)(ref_input->coeffs);
  AVX(ntt_avx)(avx_input->vec, AVX(qdata).vec);
}
...
static int correctness_random(void) {
  ...
  for (int tc = 0; tc < RANDOM_CASES; ++tc) {
    prepare_intt_inputs(&ref, &avx);

    REF(invntt_tomont)(ref.coeffs);
    AVX(invntt_avx)(avx.vec, AVX(qdata).vec);

    for (int i = 0; i < N; ++i) {
      int32_t dmod = diff_mod_q(ref.coeffs[i], avx.coeffs[i]);
      if (dmod != 0) {
        fprintf(stderr, "original intt mismatch case=%d coeff=%d\n", tc, i);
        fprintf(stderr,
                "ref=%" PRId32 " avx=%" PRId32
                " diff=%" PRId64 " diff_mod=%" PRId32 "\n",
                ref.coeffs[i], avx.coeffs[i],
                (int64_t)ref.coeffs[i] - avx.coeffs[i], dmod);
        return 0;
      }
    }
  }

  printf("  correctness: OK, %d random cases (invntt_avx vs invntt_tomont)\n",
         RANDOM_CASES);
  return 1;
}
...
static void bench_avx_cycles(const poly_t *input,
  ...
  for (size_t i = 0; i < WARMUP_ITERS; i++) {
    memcpy(work.coeffs, input->coeffs, sizeof(work.coeffs));
    AVX(invntt_avx)(work.vec, AVX(qdata).vec);
    checksum_add(&work, &acc);
  }
...
int main(void) {
  poly_t ref_input, avx_input;
  uint64_t med_avx, med_ref;
  uint64_t avg_avx = 0, avg_ref = 0;
  int64_t chk_avx = 0, chk_ref = 0;

  printf("original intt\n");

  if (!correctness_random()) {
    return 1;
  }

  prepare_intt_inputs(&ref_input, &avx_input);

  printf("  benchmark (rdtsc, %d samples each, copy+transform per sample)\n",
         NUMBER_OF_CYCLE_SAMPLES);

  bench_avx_cycles(&avx_input, ts_avx, N_CYCLE_TIMESTAMPS, &chk_avx);
  med_avx = print_results_ex("  original invntt_avx", ts_avx, N_CYCLE_TIMESTAMPS, &avg_avx);
  printf("  checksum=%" PRId64 "\n\n", chk_avx);

  bench_ref_cycles(&ref_input, ts_ref, N_CYCLE_TIMESTAMPS, &chk_ref);
  med_ref = print_results_ex("  original invntt_tomont", ts_ref, N_CYCLE_TIMESTAMPS, &avg_ref);
  printf("  checksum=%" PRId64 "\n\n", chk_ref);

  if (med_avx > 0) {
    printf("  speedup (median) invntt_tomont / invntt_avx: %.4f\n",
           (double)med_ref / (double)med_avx);
    printf("  speedup (average) invntt_tomont / invntt_avx: %.4f\n",
